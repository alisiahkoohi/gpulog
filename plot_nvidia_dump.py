...
def main():
    ...
    stats = pd.read_csv(args.statfile,
                        ...
                        index_col=0)
    stats['utilization.gpu [%]'] = stats['utilization.gpu [%]'].str.rstrip(' %').astype(float) / 100
    stats['memory.used [MiB]'] = stats['memory.used [MiB]'].str.rstrip(' MiB').astype(float)
    stats = stats.dropna()
    bus_ids = {
        bus_id: i
    ...
    stats.groupby('gpu_id')['memory.used [MiB]'].plot(ax=ax_mem, lw=1)
